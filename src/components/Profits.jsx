...
import { customerProfits } from '../constants';
import { arrow_right, minus, plus } from '../assets';
...
const Profits = () => {
    ...
    return (
        <section className='container my-16'>
            ...
            {/* For mobile */}
            {customerProfits.map((content, index) => (
                <div className="mb-4 lg:hidden" key={index}>
                    <div key={index} className="flex justify-between p-4 bg-n-9/40 backdrop-blur border border-n-1/10 rounded-2xl gap-5 mb-5 cursor-pointer" onClick={() => toggleVisibility(index)}>
                        <h5 className="flex items-center pointer-events-none">
                            <img src={content.icon} alt={content.id} className={`mr-3`} />
                            {content.title}
                        </h5>
                ...
                <div className="md:w-1/3">
                    {customerProfits.map((content, index) => (
                        <div key={index} className="flex justify-between p-4 bg-n-9/40 backdrop-blur border border-n-1/10 rounded-2xl gap-5 mb-2.5 mr-6 cursor-pointer" onClick={() => toggleVisibility(index)}>
                            ...
                            <img src={minus} alt="plus" width={30} height={30} />
                        </div>
                    ))}
                </div>
